...
const LIFECYCLE_STEPS = [
  { id: 1, name: "Create Order", service: "Order Service (localhost:8080/api/shipments)", endpoint: "POST /shipments", eventType: "ORDER_CREATED" },
  { id: 2, name: "Initialize Tracking", service: "Tracking Service (localhost:8080/api/tracking)", endpoint: "POST /track/:shipmentId/events", eventType: "ORDER_PLACED" },
  { id: 3, name: "Request Mission Allocation", service: "Mission Service (localhost:8080//api/missions)", endpoint: "POST /missions", eventType: "MISSION_ALLOCATED" },
  { id: 4, name: "Record Drone Assignment", service: "Tracking Service (localhost:8080/api/tracking)", endpoint: "POST /track/:shipmentId/events", eventType: "DRONE_ASSIGNED" },
  { id: 5, name: "Record Telemetry / Waypoint", service: "Tracking Service (localhost:8080/api/tracking)", endpoint: "POST /track/:shipmentId/events", eventType: "WAYPOINT_REACHED" },
  { id: 6, name: "Complete Mission Flight", service: "Mission Service (localhost:8080/api/missions)", endpoint: "PATCH /missions/:id/complete", eventType: "MISSION_COMPLETED" },
  { id: 7, name: "Update Order to DELIVERED", service: "Order Service (localhost:8080/api/shipments)", endpoint: "PATCH /shipments/:id/status", eventType: "STATUS_UPDATED_DELIVERED" },
  { id: 8, name: "Finalize Delivery Tracking", service: "Tracking Service (localhost:8080/api/tracking)", endpoint: "POST /track/:shipmentId/events", eventType: "DELIVERED" },
];
      ...
      if (stepNum === 1) {
        ...
        ctx.shipmentId = data.id || data.shipmentId;
          ...
          const payload = {
            ...
            type: "ShipmentPlaced",
            aggregateId: ctx.shipmentId,
            occurredAt: new Date().toISOString(),
            data: {
              additionalProp1: {}
            }
          };
          const res = await fetch(`${CONFIG.trackingService}/events`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
        ...
        const payload = {
          ...
          data: {
            droneId: ctx.droneId,
            description: `Drone ${ctx.droneId} assigned.`
          }
        };
        const res = await fetch(`${CONFIG.trackingService}/events`, {
          method: "POST",
        ...
        const payload = {
          id: `evt_${Math.random().toString(36).substring(2, 10)}`,
          type: "ShipmentStatusChanged", // Match consumer event mapping
          aggregateId: ctx.shipmentId,
          occurredAt: new Date().toISOString(),
          data: {
            status: "IN_TRANSIT",
            progress: 50,
            description: "Mid-flight waypoint passed."
          }
        };
        const res = await fetch(`${CONFIG.trackingService}/events`, {
          method: "POST",
        ...
        const res = await fetch(`${CONFIG.missionService}/missions/${ctx.missionId}/complete`, { 
          method: "POST",
          headers: { "Content-Type": "application/json" }
        });
        ...
        if (!ctx.shipmentId) throw new Error("Missing shipment ID");
        const res1 = await fetch(`${CONFIG.orderService}/shipments/${ctx.shipmentId}/status`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: "CONFIRMED" }),
        });
        logEntry.response = await res1.json();
        logEntry.status = "SUCCESS";



        if (!ctx.shipmentId) throw new Error("Missing shipment ID");
        const res2 = await fetch(`${CONFIG.orderService}/shipments/${ctx.shipmentId}/status`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: "IN_TRANSIT" }),
        });
        logEntry.response = await res2.json();
        logEntry.status = "SUCCESS";
        ...
        if (!ctx.shipmentId) throw new Error("Missing shipment ID");
        const res3 = await fetch(`${CONFIG.orderService}/shipments/${ctx.shipmentId}/status`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: "DELIVERED" }),
        });
        logEntry.response = await res3.json();
        logEntry.status = "SUCCESS";
        ...
        const payload = {
          id: `evt_${Math.random().toString(36).substring(2, 10)}`,
          type: "ShipmentDelivered", // Fixed from "Delivered" to match consumer expected type
          aggregateId: ctx.shipmentId,
          occurredAt: new Date().toISOString(),
          data: {
            progress: 100,
